...
class Model:
    def __init__(self, model_filepath: str) -> None:
        ...
        self._model_filepath.parent.mkdir(parents=True, exist_ok=True) # Ensure the folderpath exists 
    ...
    def train(self, Xy_filepath: str, n_estimators: int) -> None:
        ...
        Xy = Xy.dropna(subset=('24h_later_load')) # Only train on data for which we have the target
        X, y = Xy.drop(columns=['24h_later_load']), Xy['24h_later_load']

        # Train model
        model = lgb.LGBMRegressor(n_estimators=n_estimators, force_row_wise=True, verbose=0)
        model.fit(X, y)

        # Dump to disk
        joblib.dump(model, self._model_filepath)

    def backtest(self, Xy_filepath: str, starting_ts: pd.Timestamp, use_every_nth_ts: int = 1) -> Tuple[pd.DataFrame, float]:
    ...
    def predict(self, in_df_filepath: str, out_yhat_filepath: str) -> pd.Series:
        ...
        # Run predictions
        yhat = pd.DataFrame({
            'predicted_24h_later_load': model.predict(df.loc[timestamps].drop(columns=['24h_later_load']))
        }, index=timestamps)

        # Dump to output df
        Path(out_yhat_filepath).parent.mkdir(parents=True, exist_ok=True) # Ensure the folderpath exists 
